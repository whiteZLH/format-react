...
const MainLayout = () => {
  const navItems = getAllStrategies().map((strategy) => ({
    id: strategy.id,
    name: strategy.name,
    icon: iconMap[strategy.id as keyof typeof iconMap] ?? FileCode,
    path: `/format/${strategy.id}`,
  }));
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background text-foreground">
      <aside className="w-16 flex flex-col items-center py-4 border-r border-border bg-card/50 backdrop-blur-sm z-50">
        ...
        <nav className="flex-1 w-full flex flex-col items-center gap-4 px-2 overflow-y-auto pb-4">
          {navItems.map((item) => (
            <NavLink
              key={item.id}
              to={item.path}
              className={({ isActive }) =>
                cn(
                  "w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-200 group relative shrink-0",
                  isActive
